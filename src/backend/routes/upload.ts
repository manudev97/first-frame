    ...
    if (bot && channelId) {
      ...
      if (!channelId) {
        console.warn('⚠️  TELEGRAM_CHANNEL_ID o TELEGRAM_CHANNEL_LINK no configurado. Video no fue reenviado.');
            ...
            // IMPORTANTE: Guardar channelMessageId, videoFileId, uploaderName, uploader y tokenId en el registry para poder reenviar después
            if (channelMessageId && videoFileId) {
              try {
                ...
                if (ip) {
                  ...
                  // CRÍTICO: Guardar tokenId si está disponible (clave única para identificar el IP correcto)
                  if (tokenId && !ip.tokenId) {
                    ip.tokenId = tokenId.toString();
                    console.log(`✅ TokenId guardado para IP ${ipId}: ${ip.tokenId}`);
                  }
                  ...
                  console.log(`✅ Channel message ID, video file ID, uploaderName, uploader y tokenId guardados para IP ${ipId} (Token ID: ${ip.tokenId || 'N/A'})`);
