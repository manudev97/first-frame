            ...
            if (ip.uploader) {
              try {
                ...
                console.log(`   - ChannelMessageId: ${ip.channelMessageId || 'N/A'}`);
                 ...
                 // CRÍTICO: SIEMPRE usar el tokenId del REQUEST (del puzzle resuelto), NO el del IP encontrado
                 // El tokenId del request es la fuente de verdad porque es el tokenId del puzzle que el usuario resolvió
                 // El ip.tokenId puede ser incorrecto si el IP fue encontrado por ipId (contrato que tiene múltiples tokens)
                 const royaltyTokenId = tokenId?.toString() || correctTokenId || ip.tokenId;
                 console.log(`💰 Creando regalía con tokenId del PUZZLE RESUELTO: ${royaltyTokenId}`);
                 console.log(`   - tokenId del request (puzzle resuelto): ${tokenId || 'N/A'}`);
                 console.log(`   - correctTokenId calculado: ${correctTokenId || 'N/A'}`);
                 console.log(`   - tokenId del IP encontrado: ${ip.tokenId || 'N/A'}`);
                 console.log(`   - tokenId FINAL usado para regalía: ${royaltyTokenId}`);
                 
                 const royalty = await createPendingRoyalty(
                   telegramUserId,
                   finalIpId, // CRÍTICO: Usar el IP ID correcto, no el del contrato
                   correctTitle || ip.title || 'Video sin título', // CRÍTICO: Usar título del request (correctTitle)
                   '0.1', // Monto fijo de regalía (0.1 IP)
                   uploaderTelegramId,
                   uploaderName, // Usar nombre del registry
                   royaltyTokenId, // CRÍTICO: SIEMPRE usar tokenId del request (puzzle resuelto), NO ip.tokenId
                   ip.channelMessageId,
                   ip.videoFileId,
